...
const VERSION_FILES: VersionFile[] = [
  ...
  {
    relativePath: "apps/prisma-gui/src-tauri/tauri.conf.json",
  ...
  {
    relativePath: "apps/prisma-gui/package.json",
  ...
  {
    relativePath: "apps/prisma-gui/src-tauri/Cargo.toml",
  ...
  server.tool(
    ...
    async ({ change_type }) => {
      try {
        ...
        lines.push(
          `2. **\`apps/prisma-gui/src-tauri/tauri.conf.json\`**: Change \`"version": "${currentVersion}"\` to \`"version": "${nextVersion}"\``,
        );
        lines.push(
          `3. **\`apps/prisma-gui/package.json\`**: Change \`"version": "${currentVersion}"\` to \`"version": "${nextVersion}"\``,
        );
        lines.push(
          `4. **\`apps/prisma-gui/src-tauri/Cargo.toml\`**: Change \`version = "${currentVersion}"\` to \`version = "${nextVersion}"\` under \`[package]\``,
        );
        lines.push(``);
        lines.push(
