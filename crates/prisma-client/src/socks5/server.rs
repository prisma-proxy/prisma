...
async fn handle_connect(
    ...
    // Only override DIRECT routing for Smart DNS blocklist hits — NOT for tunnel-all mode.
    // Tunnel mode tunnels DNS resolution but respects user-defined DIRECT routing rules.
    let force_proxy = domain.is_some_and(|d| ctx.dns_resolver.smart_dns_blocks(d));
