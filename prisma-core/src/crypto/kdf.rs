...
//
// v5 uses new domain separation strings ("prisma-v5-*") to ensure complete
// cryptographic independence from v4 keys, even if the same shared secret
// is somehow reused. This is a defense-in-depth measure.
...
mod tests {
    ...
    #[test]
    fn test_v5_preliminary_differs_from_v4() {
        let secret = [0xCDu8; 32];
        let client_pub = [0x01u8; 32];
        let server_pub = [0x02u8; 32];

        let v4_prelim = derive_preliminary_key(&secret, &client_pub, &server_pub, 1000);
        let v5_prelim = derive_v5_preliminary_key(&secret, &client_pub, &server_pub, 1000);
        assert_ne!(
            v4_prelim, v5_prelim,
            "v5 preliminary key must differ from v4"
        );
    }
    ...
    #[test]
    fn test_v5_session_key_differs_from_v4() {
        let secret = [0xEFu8; 32];
        let client_pub = [0x01u8; 32];
        let server_pub = [0x02u8; 32];
        let challenge = [0x03u8; 32];

        let v4_key = derive_v3_session_key(&secret, &client_pub, &server_pub, &challenge, 1000);
        let v5_key = derive_v5_session_key(&secret, &client_pub, &server_pub, &challenge, 1000);
        assert_ne!(v4_key, v5_key, "v5 session key must differ from v4");
    }
