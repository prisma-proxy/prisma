...
Download `prisma-gui-windows-x64-setup.exe` from [GitHub Releases](https://github.com/Yamimega/prisma/releases/latest) and run the installer.
...
**Ubuntu/Debian:** `sudo dpkg -i prisma-gui_1.5.1_amd64.deb`

**AppImage:** `chmod +x prisma-gui-1.5.1.AppImage && ./prisma-gui-1.5.1.AppImage`
...
Download from [GitHub Releases](https://github.com/Yamimega/prisma/releases/latest) or TestFlight. Features: TUN mode via Network Extension, all transports, subscription management.

## Verify

```bash
prisma --version
# Expected: prisma 1.5.1
```
