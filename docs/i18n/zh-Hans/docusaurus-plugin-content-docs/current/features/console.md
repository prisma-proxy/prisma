...
每客户端带宽监控和配额管理：
- **汇总表格** -- 所有客户端的上传/下载速率限制、配额分配、已用配额和使用百分比（带颜色编码的进度条：绿色 < 70%、黄色 70-90%、红色 > 90%）
- **配额概览图表** -- 堆叠柱状图（Recharts）显示每个客户端的已用与剩余配额
- 每 5 秒自动刷新数据

### 速度测试

通过代理连接运行吞吐量测试：
- **测试运行器** -- 多流下载（4 个并行流到 Cloudflare）和上传速度测量
- **实时进度** -- 测试期间实时显示下载/上传 Mbps 和进度条
- **结果** -- 完成后以卡片形式显示下载速度、上传速度和延迟
- **历史记录** -- 持久化测试历史（最多 50 条）存储在 localStorage 中，以列表形式显示并带时间戳
- 支持中途启动和停止测试
...
- **端口转发表格** -- 活跃的端口转发，显示端口、协议、绑定地址、活跃连接数和流量计数
...
- **主题切换** -- 深色、浅色和系统模式，可从顶栏切换。偏好持久化存储在 localStorage 中，页面加载时自动应用。
