...
Prisma v2.24.0 supports Linux, macOS, Windows, FreeBSD, Docker, and mobile platforms (Android/iOS via prisma-gui). Choose the installation method that best fits your environment.

## One-Line Install

The fastest way to get Prisma running. Automatically detects your OS and architecture, downloads the v2.24.0 binary, and places it on your `PATH`.
...
Pin to v2.24.0 explicitly (or any other release tag):
...
| `--version VER` | Install a specific version (e.g., `v2.24.0`). Default: latest |
...
If you prefer to download the v2.24.0 binary directly:
...
Run the v2.24.0 server directly with Docker:
...
Pre-built binaries for v2.24.0 are available for the following targets via GitHub Releases:
