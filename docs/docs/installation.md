...
Prisma v2.6.0 supports Linux, macOS, Windows, FreeBSD, Docker, and mobile platforms (Android/iOS via prisma-gui). Choose the installation method that best fits your environment.

## One-Line Install

The fastest way to get Prisma running. Automatically detects your OS and architecture, downloads the v2.6.0 binary, and places it on your `PATH`.
...
Pin to v2.6.0 explicitly (or any other release tag):

<Tabs>
  <TabItem value="linux" label="Linux / macOS" default>

```bash
curl -fsSL https://raw.githubusercontent.com/Yamimega/prisma/master/scripts/install.sh | bash -s -- --version v2.6.0
```
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/Yamimega/prisma/master/scripts/install.ps1))) -Version v2.6.0
```
...
| `--version VER` | Install a specific version (e.g., `v2.6.0`). Default: latest |
...
If you prefer to download the v2.6.0 binary directly:

<Tabs>
  <TabItem value="linux-x64" label="Linux x86_64" default>

```bash
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.6.0/prisma-linux-amd64 -o /usr/local/bin/prisma && chmod +x /usr/local/bin/prisma
```
...
```bash
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.6.0/prisma-linux-arm64 -o /usr/local/bin/prisma && chmod +x /usr/local/bin/prisma
```
...
```bash
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.6.0/prisma-linux-armv7 -o /usr/local/bin/prisma && chmod +x /usr/local/bin/prisma
```
...
```bash
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.6.0/prisma-darwin-$(uname -m | sed s/x86_64/amd64/) -o /usr/local/bin/prisma && chmod +x /usr/local/bin/prisma
```
...
```powershell
New-Item -Force -ItemType Directory "$env:LOCALAPPDATA\prisma" | Out-Null; Invoke-WebRequest -Uri "https://github.com/Yamimega/prisma/releases/download/v2.6.0/prisma-windows-arm64.exe" -OutFile "$env:LOCALAPPDATA\prisma\prisma.exe"; [Environment]::SetEnvironmentVariable("Path", "$([Environment]::GetEnvironmentVariable('Path','User'));$env:LOCALAPPDATA\prisma", "User")
```
...
```bash
fetch -o /usr/local/bin/prisma https://github.com/Yamimega/prisma/releases/download/v2.6.0/prisma-freebsd-amd64 && chmod +x /usr/local/bin/prisma
```
...
Run the v2.6.0 server directly with Docker:

```bash
docker run --rm -v $(pwd):/config ghcr.io/yamimega/prisma:2.6.0 server -c /config/server.toml
```

Run the client:

```bash
docker run --rm -v $(pwd):/config -p 1080:1080 -p 8080:8080 ghcr.io/yamimega/prisma:2.6.0 client -c /config/client.toml
```
...
services:
  prisma-server:
    image: ghcr.io/yamimega/prisma:2.6.0
...
Pre-built binaries for v2.6.0 are available for the following targets via GitHub Releases:
