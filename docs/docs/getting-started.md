...
This guide walks you through setting up Prisma v2.8.0 and running your first proxy session — from pre-built binaries or from source.
...
Download the latest v2.8.0 release for your platform:

```bash
# Linux x86_64
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.8.0/prisma-linux-amd64 -o prisma && chmod +x prisma

# macOS (Apple Silicon)
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.8.0/prisma-darwin-arm64 -o prisma && chmod +x prisma

# macOS (Intel)
curl -fsSL https://github.com/Yamimega/prisma/releases/download/v2.8.0/prisma-darwin-amd64 -o prisma && chmod +x prisma
```
