...
Encrypted proxy system built in Rust. Workspace version 2.23.0, edition 2021.
...
| Crate | Role |
|-------|------|
| `crates/prisma-core` | Shared library: crypto (PrismaVeil v5), protocol, config (serde_json::Value compat layer), types, bandwidth, DNS, routing, congestion auto mode |
...
| `crates/prisma-mgmt` | Management API (axum): REST + WebSocket endpoints, auth middleware, SQLite persistence, subscriptions, redemption codes, invite links, settings |
| `crates/prisma-ffi` | C FFI shared library for GUI/mobile: lifecycle, profiles, QR, system proxy, auto-update with proxy support, per-app proxy |
...
- SQLite database for users, clients, routing rules, subscriptions, plans
- Subscription system: redemption codes (PRISMA-XXXX), invite links, named plans
- Console settings: registration toggle, session expiry, auto-backup interval
- Role-based dashboard (admin vs client views)
- Per-app proxy with presets, elevation detection, Windows process resolution
- Connection map (AnyChart-style choropleth + city dots + zoom/pan)
- Extended traffic charts (1h-30d, top-10 multi-line, CSV export)
- Enriched system tray (speed stats, quick toggles, update check, recent connections)
- Config forward/backward compatibility (preserve unknown TOML fields, enum fallbacks, bidirectional SQLite migrations)
- GeoSite routing with auto-download + category presets
- Routing rules: serde_json::Value deserializer (works with both JSON + TOML)
