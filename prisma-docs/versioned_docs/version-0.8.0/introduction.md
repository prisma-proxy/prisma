...
Prisma is a next-generation encrypted proxy infrastructure suite built in Rust. It implements the **PrismaVeil v5** wire protocol — combining modern cryptography, multiple transport options, and advanced anti-censorship features.

## Features

- **PrismaVeil v5 protocol** — 1-RTT handshake, 0-RTT resumption, X25519 + BLAKE3 + ChaCha20-Poly1305 / AES-256-GCM / Transport-Only cipher modes, header-authenticated encryption (AAD), connection migration, enhanced KDF (backward compatible with v4), post-quantum readiness
- **9 transports** — QUIC v2, TCP, WebSocket, gRPC, XHTTP, XPorta, ShadowTLS v3, SSH, WireGuard — with auto-fallback
...
- **SSH transport** — tunnels through standard SSH connections for maximum compatibility
- **WireGuard transport** — uses WireGuard protocol for kernel-level performance
...
- **Subscription management** — multi-protocol import (SS/VMess/Trojan/VLESS) with auto-update
...
- **io_uring support** — zero-copy I/O on Linux for maximum throughput
- **Mobile FFI** — C ABI shared library for GUI and mobile app integration (Android/iOS)
...
├── prisma-ffi/        # C FFI shared library for GUI/mobile integration
