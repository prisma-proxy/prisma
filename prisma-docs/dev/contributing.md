...
1. **Client stream adapter:** `prisma-client/src/my_transport_stream.rs` -- implement `AsyncRead + AsyncWrite + Send + Unpin`
2. **Register in transport selector:** `prisma-client/src/transport_selector.rs` -- add match arm
3. **Add module:** `prisma-client/src/lib.rs`
4. **Server listener:** `prisma-server/src/listener/my_transport.rs` -- implement `listen()` function
5. **Register listener:** `prisma-server/src/listener/mod.rs` and `prisma-server/src/lib.rs`
6. **Add config fields:** `prisma-core/src/config/server.rs` and `client.rs`
7. **Update version command:** `prisma-cli/src/main.rs`

---

## Adding a New CLI Command

1. Add command variant to `Commands` enum in `prisma-cli/src/main.rs`
2. Create handler module in `prisma-cli/src/`
...
2. Register route in `prisma-mgmt/src/router.rs`
