...
1. **在 `prisma-client/src/` 创建客户端流适配器：**

   ```
   prisma-client/src/my_transport_stream.rs
   ```

   实现 `AsyncRead + AsyncWrite + Send + Unpin` 的结构体。

2. **在传输选择器中注册** (`prisma-client/src/transport_selector.rs`)

3. **在 `prisma-client/src/lib.rs` 添加模块**

4. **在 `prisma-server/src/listener/` 创建服务端监听器**

5. **在服务端注册** (`prisma-server/src/listener/mod.rs` 和 `prisma-server/src/lib.rs`)

6. **添加配置字段**

7. **更新版本命令** (`prisma-cli/src/main.rs`)

### 需要修改的文件

| 文件 | 修改 |
|------|------|
| `prisma-client/src/lib.rs` | 添加模块 |
| `prisma-client/src/my_transport_stream.rs` | 新文件 |
| `prisma-client/src/transport_selector.rs` | 添加匹配分支 |
| `prisma-server/src/listener/mod.rs` | 添加模块 |
| `prisma-server/src/listener/my_transport.rs` | 新文件 |
| `prisma-server/src/lib.rs` | 启动监听器 |
| `prisma-core/src/config/server.rs` | 添加配置结构体 |
| `prisma-core/src/config/client.rs` | 添加配置字段 |
...
1. 在 `prisma-mgmt/src/handlers/` 创建处理器
2. 在 `prisma-mgmt/src/router.rs` 注册路由
3. 可选添加 CLI 命令调用该端点
