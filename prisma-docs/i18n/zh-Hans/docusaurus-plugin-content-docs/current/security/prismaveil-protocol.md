...
| 版本 | 握手 | 特性 |
|------|------|------|
| **v4 (0x04)** | **2 步（1 RTT）** | 所有功能：0-RTT 恢复、PrismaUDP、FEC、DNS、速度测试、拥塞控制、端口跳变、Salamander v2、桶填充、流量整形、PrismaTLS、PrismaFP、熵伪装 |
...
剥离 QUIC 头部，使用 BLAKE3 派生的密钥流对所有 UDP 数据包进行 XOR 混淆。线路上的流量表现为随机字节。在 v4 中，Salamander 使用逐包 nonce（非确定性）防止关联攻击，并添加 ASCII 前缀用于 GFW 熵分析（Ex2）豁免。
