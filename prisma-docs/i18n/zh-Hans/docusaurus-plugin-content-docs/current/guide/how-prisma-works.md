...
ShadowTLS 模拟与掩护服务器（如热门网站）的真实 TLS 握手。初始握手与真正的 TLS 连接无法区分，使其极难被协议检测识别。

- **优点：** 通过 DPI 检测时表现为真实 TLS 流量，极难指纹识别
- **缺点：** 需要一个支持 TLS 的掩护服务器

### SSH —— 通用兼容

SSH 传输通过标准 SSH 连接隧道传输 Prisma 流量。SSH 几乎不会被封锁，因为它对服务器管理来说是必不可少的。

- **优点：** 几乎不会被封锁，广泛可用
- **缺点：** SSH 流量模式可能被高级 DPI 识别

### WireGuard —— 内核级性能

WireGuard 是一种以简洁和高性能著称的现代 VPN 协议。Prisma 可以使用 WireGuard 作为传输层，实现内核级转发速度。

- **优点：** 非常快速，开销极小，经过充分审计的协议
- **缺点：** 使用 UDP（可能被封锁），WireGuard 可被 DPI 检测
...
| 多种传输 | 通常 1-2 种 | 1-2 种 | 9 种传输方式，自动回退 |
...
- Prisma 有**九种传输方式**（QUIC、TCP、WebSocket、gRPC、XHTTP、XPorta、ShadowTLS v3、SSH、WireGuard）适用于不同场景
