...
PrismaVeil v4 包含针对封装 TLS 指纹识别的流量整形防御机制，相关研究发表于 USENIX Security 2024。这些防御措施可防止审查者通过分析数据包大小分布和时序模式来识别代理流量。
...
- 桶填充：带宽开销低于 5%
- 杂音注入：约每 500ms 一个小帧（可忽略不计）
- 时序抖动：仅在握手阶段增加不超过 50ms 的延迟
- 帧合并：小帧增加不超过 5ms 的延迟
