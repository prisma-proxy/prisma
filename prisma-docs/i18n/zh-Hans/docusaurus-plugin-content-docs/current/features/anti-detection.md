...
PrismaVeil 包含多层反检测机制，可抵御深度包检测 (DPI)、主动探测和审查系统（如 GFW）的流量分析。
...
Salamander v2 在每个混淆数据包前添加短 ASCII 前缀。这降低了数据包的测量熵，以通过 GFW Ex2（熵分析）检查——该检查会标记纯随机流量。ASCII 前缀从数据包 nonce 确定性派生，接收方可以无歧义地剥离。
...
PrismaTLS 取代 REALITY 协议，提供主动探测抵抗能力。它将认证标签隐藏在 TLS padding 扩展中，而非已弃用的 Session ID 字段。

- **PrismaAuth**: 通过 padding 扩展信标进行身份验证（基于时间窗口轮换，位置随机化）
- **PrismaMask**: 动态掩护服务器池，自动健康检查和故障切换
- **PrismaFP**: 字节级 ClientHello 构建，匹配 Chrome/Firefox/Safari 浏览器指纹
- **PrismaFlow**: HTTP/2 SETTINGS 帧模拟 + RTT 归一化

## 流量整形

防御封装TLS指纹识别（USENIX Security 2024）：

- **桶填充**: 将帧填充到固定大小集合中的最近尺寸
- **杂音注入**: 空闲期间发送虚假帧作为背景噪声
- **时序抖动**: 对握手阶段帧添加随机延迟
- **帧合并**: 在时间窗口内缓冲小帧并合并
