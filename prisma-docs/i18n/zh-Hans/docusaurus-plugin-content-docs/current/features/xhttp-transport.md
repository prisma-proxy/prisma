...
| 特性 | QUIC | TCP | WebSocket | gRPC | XHTTP |
|------|------|-----|-----------|------|-------|
| 看起来像正常 HTTP | 否 | 否 | Upgrade 头部 | grpc content-type | 是 |
| CDN 兼容 | 否 | 否 | 是 | 部分 | 是 |
| 单连接开销 | 低 | 低 | 中 | 中 | 低（stream-one） |
| 多种上传模式 | 否 | 否 | 否 | 否 | 3 种模式 |
| SSE 伪装 | 不适用 | 不适用 | 不适用 | 不适用 | 是 |
| 头部混淆 | 不适用 | 不适用 | 基本 | 否 | 完整 |
